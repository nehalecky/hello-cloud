...
class TestCloudResourceTaxonomy:
    ...
    def test_get_nonexistent_archetype(self):
        """Test that getting non-existent archetype raises KeyError."""
        with pytest.raises(KeyError):
            CloudResourceTaxonomy.get_archetype("NonExistentArchetype")
    ...
    def test_export_for_ml_training(self):
        ...
        if CloudResourceTaxonomy.ARCHETYPES:
            # Should have features for each archetype
            assert len(ml_features) == len(CloudResourceTaxonomy.ARCHETYPES)

            # Check structure of first feature set
            first_key = next(iter(ml_features.keys()))
            features = ml_features[first_key]

            # Should have expected keys
            expected_keys = {
                'cpu_p50', 'cpu_p95', 'memory_p50', 'memory_p95',
                'cpu_cv', 'memory_cv', 'waste_percentage',
                'optimization_difficulty', 'business_criticality'
            }
    ...
    def test_compute_priors(self):
        ...
        # Should have specific prior types
        expected_priors = {'cpu_mu', 'cpu_sigma', 'memory_mu', 'memory_sigma', 'waste_alpha', 'waste_beta'}
...
class TestIntegration:
    ...
    def test_realistic_utilization_patterns(self):
        ...
        if all_cpu_p50:  # Only test if we have archetypes
            ...
            # Average memory should be around 20% (research finding)
            avg_memory = sum(all_memory_p50) / len(all_memory_p50)
            assert 10 <= avg_memory <= 40
