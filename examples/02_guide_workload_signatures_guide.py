...
hourly_stats = (
    web_app_data.groupby("hour")
    .agg({"cpu_utilization": "mean", "memory_utilization": "mean", "waste_percentage": "mean"})
    .rename(
        columns={
...
hourly_stats["period_explanation"] = np.select(
    conditions, choices, default="Evening - Reduced activity"
)

web_app_chart = (
    alt.Chart(hourly_stats)
...
weekly_pattern = (
    dev_env.groupby("weekday")
    .agg({"cpu_utilization": "mean", "waste_percentage": "mean"})
    .rename(columns={"cpu_utilization": "cpu_mean", "waste_percentage": "waste_mean"})
    .reset_index()
)
