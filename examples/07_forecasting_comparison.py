...
print(
    f"  Total: {len(train_df) + len(test_df):,} samples (~{(len(train_df) + len(test_df))/1440:.1f} days)"
)
...
print("Data splits (from subsampled training data):")
...
n_viz = 5000
axes[0].plot(
    np.arange(n_viz), y_train_full[:n_viz], "k-", linewidth=0.5, alpha=0.5, label="Full data"
)
axes[0].scatter(
    subsample_indices[: n_viz // subsample_factor],
...
horizons = {"12 steps (~12 minutes)": 12, "62 steps (~1 hour)": 62, "250 steps (~4 hours)": 250}
...
for horizon_name, horizon in horizons.items():
    ...
    if len(y_val_split) >= horizon:
        ...
        naive_results[horizon_name] = {"forecast": forecast, "y_true": y_true, "metrics": metrics}
...
for idx, (horizon_name, result) in enumerate(naive_results.items()):
    ...
    context_window = 100
    ax.plot(
        np.arange(-context_window, 0),
        y_train_split[-context_window:],
        "k-",
        linewidth=1.5,
        label="Training data",
        alpha=0.7,
    ...
    ax.plot(
        forecast_steps, result["forecast"], "r--", linewidth=2, label="Naive forecast", alpha=0.8
    )
...
for idx, (period_name, result) in enumerate(seasonal_results.items()):
    ...
    ax.plot(forecast_steps, result["y_true"], "b-", linewidth=2, label="Actual", alpha=0.8)
    ax.plot(
        forecast_steps, result["forecast"], "g--", linewidth=2, label="Seasonal Naive", alpha=0.8
    )
