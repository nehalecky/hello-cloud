...
print("⚠ Timestamp limitation: Dataset provides no real timestamps")
print("  Using sequential index as proxy (assumed 1-minute sampling)")
...
print("=" * 70)
ks_df = pl.DataFrame([{"Comparison": comp, **results} for comp, results in ks_results_dict.items()])
ks_df
...
kl_df = pl.DataFrame([{"Comparison": comp, **results} for comp, results in kl_results_dict.items()])
kl_df
...
).with_columns(pl.arange(0, len(train_df) + len(test_df)).alias("timestamp"))
...
if USE_REAL_TIME:
    print("  (Based on documented 1-minute sampling interval)")
print()
...
if len(valid_frequencies) > 0:
    print(
        f"Frequency range: {valid_frequencies.min():.6f} - {valid_frequencies.max():.6f} cycles/{TIME_UNIT}"
    )
    ...
    print(
        f"   Raw period range: {periods[frequencies > 0].min():.1f} - {periods[frequencies > 0].max():.1f} {TIME_UNIT}"
    )
...
print("✓ Autocorrelation structure maintained at key lags")
print("✓ Distribution shape preserved")
print("\n→ Subsampled data is suitable for computational efficiency")
print("   without sacrificing signal characteristics")
