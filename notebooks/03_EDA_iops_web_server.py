...
pd.DataFrame(
    {
        ...
        "Value": [kpi_id, "TSB-UAD/IOPS (AutonLab/Timeseries-PILE)"],
    }
)
...
pd.DataFrame(
    {
        ...
        "Anomaly Rate (%)": [
            100 * train_anomalies / train_count,
            100 * test_anomalies / test_count,
        ],
    }
)
...
train_stats = (
    train_df.select("value")
    .summary("count", "mean", "stddev", "min", "25%", "50%", "75%", "max")
    .collect()
)
test_stats = (
    test_df.select("value")
    .summary("count", "mean", "stddev", "min", "25%", "50%", "75%", "max")
    .collect()
)
...
base = alt.Chart(combined_viz).encode(x=alt.X("timestamp:Q", title="Time Index"))
...
anomaly_points = (
    alt.Chart(anomalies_viz)
    .mark_circle(size=60, color="red")
    .encode(x="timestamp:Q", y="value:Q", tooltip=["timestamp:Q", "value:Q"])
)

# Combine
chart = (
    (lines + anomaly_points)
    .properties(width=800, height=300, title="IOPS KPI Time Series with Labeled Anomalies")
    .interactive()
)
...
anomaly_zoom_points = (
    alt.Chart(anomalies_zoom)
    .mark_circle(size=80, color="red")
    .encode(x="timestamp:Q", y="value:Q", tooltip=["timestamp:Q", "value:Q", "label:N"])
)

chart_zoom = (
    (line_zoom + anomaly_zoom_points)
    .properties(width=800, height=300, title="Training Data - First 1,000 Points (Detailed View)")
    .interactive()
)
...
pd.DataFrame(
    {
        ...
        "Mean": [
            data_segments["normal"].mean(),
            data_segments["anomaly"].mean() if len(data_segments["anomaly"]) > 0 else None,
        ],
        "Std": [
            data_segments["normal"].std(),
            data_segments["anomaly"].std() if len(data_segments["anomaly"]) > 0 else None,
        ],
        "Min": [
            data_segments["normal"].min(),
            data_segments["anomaly"].min() if len(data_segments["anomaly"]) > 0 else None,
        ],
        "Max": [
            data_segments["normal"].max(),
            data_segments["anomaly"].max() if len(data_segments["anomaly"]) > 0 else None,
        ],
    }
)
...
print("=" * 70)
ks_df = pd.DataFrame([{"Comparison": comp, **results} for comp, results in ks_results_dict.items()])
ks_df
...
kl_df = pd.DataFrame([{"Comparison": comp, **results} for comp, results in kl_results_dict.items()])
kl_df
...
full_df = full_df.withColumn(
    "timestamp", F.row_number().over(Window.orderBy(F.monotonically_increasing_id())) - 1
)
...
acf_chart = (
    ...
    .properties(width=700, height=250, title="Autocorrelation Function (ACF)")
)

# Add confidence interval lines
ci_upper = (
    alt.Chart(pd.DataFrame({"y": [ci]})).mark_rule(color="red", strokeDash=[5, 5]).encode(y="y:Q")
)
ci_lower = (
    alt.Chart(pd.DataFrame({"y": [-ci]})).mark_rule(color="red", strokeDash=[5, 5]).encode(y="y:Q")
)
        ...
        peak_data.append(
            {
                ...
                "Interpretation": (
                    "Very Strong"
                    if peak_prominence[idx] > 0.3
                    else (
                        "Strong"
                        if peak_prominence[idx] > 0.15
                        else "Moderate"
                        if peak_prominence[idx] > 0.05
                        else "Weak"
                    )
                ),
            }
...
if len(valid_frequencies) == 0 or len(valid_psd) == 0:
    ...
    ax.loglog(
        valid_frequencies, valid_psd, "k-", linewidth=1.5, alpha=0.7, label="Power Spectral Density"
    )
    ...
    if len(peak_indices) > 0:
        ...
        ax.scatter(
            ...
            alpha=0.7,
            zorder=5,
            label=f"{len(peak_indices)} Detected Peaks",
            marker="o",
        )
    ...
    if len(valid_psd) > 0:
        ...
        ax.axhline(
            ...
            linewidth=2,
            alpha=0.5,
            label="Significance Threshold (5%)",
        )

    ax.set_xlabel(f"Frequency (cycles/{TIME_UNIT})", fontsize=12)
    ax.set_ylabel("Power Spectral Density", fontsize=12)
    title_suffix = (
        f" (Full Dataset: {len(full_values):,} samples)" if "full_values" in locals() else ""
    )
    ax.set_title(
        f"Welch Power Spectral Density - Periodicity Detection{title_suffix}",
        fontsize=14,
        fontweight="bold",
    )
...
if len(peak_indices) > 0:
    ...
    print("  (Using training data only to avoid test leakage)")
    ...
    try:
        ...
        strength_label = (
            ...
            else ("Moderate" if strength_seasonal > 0.3 else "Weak")
        )
        ...
        logger.info(
            f"STL decomposition successful: {strength_label} seasonality ({strength_seasonal:.3f})"
        )
...
if len(peak_indices) > 0:
    ...
    try:
        ...
        ).properties(title=f"STL Decomposition (Period={period})")
    except:
...
if len(peak_indices) > 0:
    ...
    logger.info(
        f"Nyquist-safe subsampling: factor={subsample_factor} (from max freq={highest_freq:.6f})"
    )
else:
    subsample_factor = 30
...
logger.info(
    f"Subsampling: {len(train_values):,} → {len(train_values_sub):,} samples ({100*(1-len(train_values_sub)/len(train_values)):.1f}% reduction)"
)
...
stats_comparison["Diff %"] = (
    (stats_comparison["Subsampled"] - stats_comparison["Full Data"])
    / stats_comparison["Full Data"]
    * 100
)
    ...
    acf_comparison.append(
        {
            ...
            "Preserved": (
                "Yes"
                if not np.isnan(acf_full)
                and not np.isnan(acf_sub)
                and abs(acf_full - acf_sub) < 0.1
                else "N/A"
            ),
        }
    )
    ...
    interpretation_df = pd.DataFrame(
        {
            ...
            "Options": [
                "1) Difference the series (ARIMA) | 2) Detrend (Prophet, STL) | 3) Use trend-aware methods | 4) Foundation models (handle non-stationarity)"
            ],
        }
    )
...
gp_suitability = (
    "✅ Feasible"
    if n_train <= 10000
    else ("⚠️ Use sparse GP" if n_train <= 50000 else "❌ Computationally expensive")
)
...
horizon_data = {
    ...
    "Use Case": ["Immediate anomaly detection", "Operational planning", "Capacity planning"],
    "Expected Accuracy": [
