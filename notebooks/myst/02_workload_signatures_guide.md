...
alt.themes.enable('quartz')  # Clean, professional theme
...
io_wait_demo = pl.DataFrame({
    'time': list(range(100)),
    'cpu_active': [20 + 10*np.sin(i/10) + np.random.normal(0, 3) for i in range(100)],
    'io_wait': [15 + 8*np.sin(i/10 + np.pi) + np.random.normal(0, 2) for i in range(100)]
})

io_wait_demo = io_wait_demo.with_columns([
    (100 - pl.col('cpu_active') - pl.col('io_wait')).alias('idle')
])

io_chart = alt.Chart(io_wait_demo.to_pandas()).transform_fold(
    ['cpu_active', 'io_wait', 'idle'],
    as_=['component', 'percentage']
).mark_area().encode(
