...
cutoff_date_result = (
    daily_with_change.filter(F.col("pct_change") < -30.0)
    .orderBy("date")
    .limit(1)
    .select("date")
    .toPandas()
...
if categorical_cols:
    ...
    plt.suptitle(
        "Categorical Value Distributions (Top 10 per column, log scale)",
        fontsize=14,
        fontweight="bold",
        y=1.0,
    )
...
fig = hc.utils.plot_dimension_cost_summary(
    df, dimensions=dimensions, cost_col="cost", top_n=10, cols_per_row=2
)
plt.show()
...
fig = hc.utils.plot_grain_persistence_comparison(
    grain_comparison, stability_threshold=70.0, figsize=(14, 8)
)
plt.show()
...
# Output directory
OUTPUT_DIR = Path(
    "~/Projects/cloudzero/cloud-resource-simulator/data/piedpiper_processed"
).expanduser()
...
logger.info("   Temporal range: Sept 1 - Oct 6, 2025 (37 days)")
logger.info("   Format: (date, [categorical attributes], cost)")
logger.info("\n📊 Dataset contains:")
logger.info("   • Filtered schema (high-info columns only)")
logger.info("   • Temporal filter (clean period, no AWS pipeline collapse)")
logger.info("   • Primary cost metric (materialized_cost → cost)")
logger.info("   • Hierarchical attributes (use functional dependency analysis)")
