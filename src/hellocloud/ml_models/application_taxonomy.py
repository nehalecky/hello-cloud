...
A hierarchical classification of cloud workloads based on:
...
class ResourcePattern(BaseModel):
    ...
    seasonality_strength: float = Field(
        ..., ge=0, le=1, description="Strength of seasonal patterns"
    )
...
class CostProfile(BaseModel):
    ...
    optimization_difficulty: float = Field(
        ..., ge=0, le=1, description="How hard to optimize (0-1)"
    )
    business_criticality: float = Field(
        ..., ge=0, le=1, description="Impacts over-provisioning (0-1)"
    )
...
class CloudResourceTaxonomy:
    ...
    @classmethod
    def get_by_optimization_potential(
        cls, potential: OptimizationPotential
    ) -> list[ApplicationArchetype]:
    ...
    @classmethod
    def compute_priors(cls) -> dict[str, dict[str, float]]:
        """Compute statistical priors from the taxonomy for Bayesian modeling"""
        # Compute basic statistics
        cpu_p50_values = [a.resource_pattern.cpu_p50 for a in cls.ARCHETYPES.values()]
        memory_p50_values = [a.resource_pattern.memory_p50 for a in cls.ARCHETYPES.values()]

        priors = {
            "cpu_utilization": {
                "mean": np.mean([a.resource_pattern.cpu_p50 for a in cls.ARCHETYPES.values()]),
                "std": np.std([a.resource_pattern.cpu_p50 for a in cls.ARCHETYPES.values()]),
                "min": min(a.resource_pattern.cpu_p50 for a in cls.ARCHETYPES.values()),
                "max": max(a.resource_pattern.cpu_p95 for a in cls.ARCHETYPES.values()),
            },
            "memory_utilization": {
                "mean": np.mean([a.resource_pattern.memory_p50 for a in cls.ARCHETYPES.values()]),
                "std": np.std([a.resource_pattern.memory_p50 for a in cls.ARCHETYPES.values()]),
                "min": min(a.resource_pattern.memory_p50 for a in cls.ARCHETYPES.values()),
                "max": max(a.resource_pattern.memory_p95 for a in cls.ARCHETYPES.values()),
            },
            "waste_percentage": {
                "mean": np.mean([a.cost_profile.waste_percentage for a in cls.ARCHETYPES.values()]),
                "std": np.std([a.cost_profile.waste_percentage for a in cls.ARCHETYPES.values()]),
                "min": min(a.cost_profile.waste_percentage for a in cls.ARCHETYPES.values()),
                "max": max(a.cost_profile.waste_percentage for a in cls.ARCHETYPES.values()),
            },
            "hourly_cost": {
                "mean": np.mean([a.cost_profile.avg_hourly_cost for a in cls.ARCHETYPES.values()]),
                "std": np.std([a.cost_profile.avg_hourly_cost for a in cls.ARCHETYPES.values()]),
                "min": min(a.cost_profile.avg_hourly_cost for a in cls.ARCHETYPES.values()),
                "max": max(a.cost_profile.avg_hourly_cost for a in cls.ARCHETYPES.values()),
            },
            # Additional keys expected by tests
            "cpu_mu": np.mean(cpu_p50_values),
            "memory_mu": np.mean(memory_p50_values),
            "cpu_sigma": np.std(cpu_p50_values),
            "memory_sigma": np.std(memory_p50_values),
        }
        return priors


def generate_synthetic_workload(
    archetype_name: str, duration_hours: int = 720, sampling_interval_minutes: int = 5
) -> pl.DataFrame:
    """
    ...
    timestamps = [
        start_time + timedelta(minutes=i * sampling_interval_minutes) for i in range(num_samples)
    ]
    ...
    df = pl.DataFrame(
        {
            ...
            "disk_iops": samples[:, 4],
            "hourly_cost": archetype.cost_profile.avg_hourly_cost
            * (1 + np.random.normal(0, 0.2, num_samples)),
            "waste_percentage": archetype.cost_profile.waste_percentage
            * (1 + np.random.normal(0, 0.1, num_samples)),
        }
    )
